...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Agar pending join code hai toh wahan bhejo
      const pendingCode = localStorage.getItem('wt_pending_join');
      if (pendingCode) {
    ...
    <div className="auth-bg min-h-screen flex items-center justify-center px-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-netflix-gray text-sm">Watch movies with friends, in sync</p>
        </div>
        ...
        <div className="bg-surface rounded-2xl border border-white/5 p-8 shadow-2xl">
          ...
          <p className="text-center text-netflix-gray text-sm mt-6">
            ...
            <Link to="/register" className="text-netflix-red hover:text-red-400 font-medium transition-colors">
              Create one
            </Link>
          </p>
        </div>
